...
export function useBridge(): UseBridgeReturn {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to connect bridge';
  ...
  const getStudentList = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to get student list';
      setError(errorMessage);
   ...
   * 選擇學生
   */
  const pickStudent = useCallback(async (studentId: string): Promise<boolean> => {
    const bridge = initBridge();
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to pick student';
      setError(errorMessage);
