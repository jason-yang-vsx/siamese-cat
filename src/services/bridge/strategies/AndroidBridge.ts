...
export class AndroidBridge extends BridgeStrategy {
  ...
  async connect(): Promise<void> {
    try {
      ...
      if (this.win.Android) {
        ...
        // 檢查可用的方法
        const methods = ['getStudentList', 'studentPicked', 'studentRemoved', 'receiveMessage'];
        const available = methods.filter(m => typeof (this.win.Android as any)[m] === 'function');
  ...
  async getStudentList(): Promise<Student[]> {
    ...
    if (this.win.Android?.receiveMessage) {
      ...
      const promise = this.createPendingRequest<Student[]>(message.messageId!, 5000);
  ...
  async studentPicked(event: StudentPickedEvent): Promise<boolean> {
    ...
    if (this.win.Android?.receiveMessage) {
      ...
      const promise = this.createPendingRequest<boolean>(message.messageId!, 5000);
